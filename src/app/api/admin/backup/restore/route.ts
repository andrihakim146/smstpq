        ...
        if (mode === 'skip') {
          const res = await prisma.santri.createMany({
            data: chunk.map((s) => ({
              ...
              jenisKelamin:       (s.jenisKelamin as 'LAKI_LAKI' | 'PEREMPUAN' | null | undefined) ?? null,
              usia:               (s.usia as number | null | undefined) ?? null,
              namaWali:           (s.namaWali as string | null | undefined) ?? null,
              alamat:             (s.alamat as string | null | undefined) ?? null,
